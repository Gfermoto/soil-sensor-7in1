...
bool validateConfigInput(bool checkRequired) {
    ...
    if (webServer.hasArg("ntp_interval")) {
        ...
        if (interval < 10000 || interval > 86400000) { // от 10 сек до 24 часов
    ...
    if (webServer.hasArg("sensor_read")) {
        ...
        if (interval < 1000 || interval > 300000) { // от 1 сек до 5 мин
    ...
    if (webServer.hasArg("mqtt_publish")) {
        ...
        if (interval < 1000 || interval > 3600000) { // от 1 сек до 1 часа
    ...
    if (webServer.hasArg("thingspeak_interval")) {
        ...
        if (interval < 15000 || interval > 7200000) { // от 15 сек до 2 часов (лимит ThingSpeak)
...
String generateValidationErrorResponse(const String& errorMsg) {
    ...
    content += "<p><a href='javascript:history.back()' style='color: #4CAF50; text-decoration: none;'>← Назад к форме</a></p>";
