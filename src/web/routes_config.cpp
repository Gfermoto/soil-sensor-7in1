...
void setupConfigRoutes() {
    ...
    webServer.on("/config_manager", HTTP_GET, []() {
        ...
        if (currentWiFiMode == WiFiMode::AP) {
            String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>" UI_ICON_FOLDER " Конфигурация</title>";
            html += "<style>" + String(getUnifiedCSS()) + "</style></head><body><div class='container'>";
        ...
        html += generateButton(ButtonType::PRIMARY, "📥", "Скачать конфигурацию", "location.href='/api/config/export'");
        html += "</div>";
