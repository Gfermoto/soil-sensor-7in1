...
void setupServiceRoutes() {
    ...
    webServer.on("/service", HTTP_GET, []() {
        ...
        html += "<div class='section' style='margin-top:15px;font-size:14px;color:#555'><b>API:</b> <a "
        ...
        html += "html+=dot(d.wifi_connected)+'<b>WiFi:</b> '+(d.wifi_connected?'Подключено ('+d.wifi_ip+', "
        ...
        html += "html+=dot(d.sensor_ok)+'<b>Датчик:</b> '+(d.sensor_ok?'Ок':'Ошибка'+(d.sensor_last_error?' "
                "('+d.sensor_last_error+')':''));";
    ...
    webServer.on("/reset", HTTP_POST, []() {
        ...
        resetConfig();
        String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><meta http-equiv='refresh' "
