    ...
    webServer.on("/updates", HTTP_GET, []()
                 {
                     ...
                     html += "      isOtaActive = true;\n";
                     html += "    } else if (status === 'Подключение' || status === 'Загрузка' || status === 'Проверка' || status === 'Завершение') {\n";
                     html += "      progressBar.style.display = 'block';\n";
                     html += "      progressFill.style.width = '25%';\n";
                     html += "      isOtaActive = true;\n";
                     html += "    } else {\n";
                     html += "      progressBar.style.display = 'none';\n";
                     html += "      if (isOtaActive && (status.includes('Ошибка') || status.includes('Таймаут'))) {\n";
                     html += "        showToast('OTA ошибка: ' + status, 'error');\n";
                     html += "      } else if (isOtaActive && status.includes('Успешно')) {\n";
