...
void setupDataRoutes()
{
    ...
    webServer.on(
        ...
        []()
        {
            ...
            // ----- Форма загрузки CSV -----
            html +=
                "<form action='/readings/upload' method='post' enctype='multipart/form-data' style='margin-top:10px'>";
            html +=
                "<div class='section'><h3>Загрузить CSV</h3><input type='file' name='calibration_csv' accept='.csv' "
                "required></div>";
            ...
            html +=
                "<style>.data{width:100%;border-collapse:collapse}.data th,.data td{border:1px solid "
