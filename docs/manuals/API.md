...
REST API для интеграции с JXCT Soil Sensor v3.9.0
...
- [Доступ к API](#Dostup-k-api)
  - [Таблица актуальных эндпоинтов (API v3.9.0)](#Tablitsa-aktualnyh-endpointov-api-v3.9.0)
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET   | `/api/v3.9.0/sensor`         | Основные данные датчика (JSON) |
| GET   | `/api/v3.9.0/system/health`  | Полная диагностика устройства  |
| GET   | `/api/v3.9.0/system/status`  | Краткий статус сервисов        |
| POST  | `/api/v3.9.0/system/reset`   | Сброс настроек (307 на `/reset`) |
| POST  | `/api/v3.9.0/system/reboot`  | Перезагрузка (307 на `/reboot`) |
| GET   | `/api/v3.9.0/config/export`  | Скачать конфигурацию (JSON, без паролей) |
| POST  | `/api/v3.9.0/config/import`  | Импорт конфигурации            |
...
| GET   | `/api/sensor`            | DEPRECATED alias → `/api/v3.9.0/sensor` |
| GET   | `/api/config/export`     | DEPRECATED alias → `/api/v3.9.0/config/export` |
| POST  | `/api/config/import`     | DEPRECATED alias → `/api/v3.9.0/config/import` |
