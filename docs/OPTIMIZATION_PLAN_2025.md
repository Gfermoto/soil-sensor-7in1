...
### 1. 🧶 **Замена String на статические буферы**
**Проблема:** 45+ использований String класса
**Эффект:** -20-40KB RAM, +20-30% производительности

#### Целевые файлы:
- `src/wifi_manager.cpp` (35+ String объектов)
- `src/mqtt_client.cpp` (25+ String операций)
- `src/thingspeak_client.cpp` (7+ String conversions)
...
- [ ] Замена String на статические буферы
...
|----------|---------------|------------------|-----------|
| **RAM использование** | ~200KB | ~140KB | **-30%** |
| **Flash размер** | ~1000KB | ~750KB | **-25%** |
| **CPU нагрузка** | ~50% | ~35% | **-30%** |
| **Время отклика** | ~500ms | ~300ms | **-40%** |
| **Энергопотребление** | ~175mA | ~120mA | **-31%** |
