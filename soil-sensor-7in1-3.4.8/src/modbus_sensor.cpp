...
static void applyCompensationIfEnabled(SensorData& d)
{
    ...
    // 1. EC: применяем модель Арчи (температурная компенсация уже внутри correctEC)
    d.ec = correctEC(d.ec, d.temperature, d.humidity, soil);

    // 2. pH: температурная поправка
    d.ph = correctPH(d.ph, d.temperature);

    // 3. NPK: применяем FAO 56 + Eur. J. Soil Sci. с учетом условий выращивания
    float nCalibrated = d.nitrogen;
    float pCalibrated = d.phosphorus;
    float kCalibrated = d.potassium;
    correctNPK(d.temperature, d.humidity, nCalibrated, pCalibrated, kCalibrated, soil, config.environmentType);

    // ИСПРАВЛЕНО: Убираем NPK_FACTOR - датчик JXCT уже выдаёт данные в мг/кг
    // согласно официальной документации: https://www.jxct-iot.com/product/showproduct.php?id=197
