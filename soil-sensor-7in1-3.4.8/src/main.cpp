...
void loop()
{
    ...
    static bool firstValidDataReceived = false;  // Флаг для отслеживания первых валидных данных
    static bool dataWasValid = false;  // Флаг для отслеживания предыдущего состояния валидности
    static bool invalidDataPublished = false;  // Флаг для предотвращения повторной публикации невалидных данных

    // Проверяем изменение состояния валидности данных
    if (sensorData.valid != dataWasValid)
    {
        if (!sensorData.valid && dataWasValid && !invalidDataPublished)
        {
            // Данные стали невалидными - публикуем последние валидные значения
            DEBUG_PRINTLN("[INVALID] Данные стали невалидными - публикуем последние валидные значения");
            
            // Принудительная публикация MQTT
            if (config.flags.mqttEnabled)
            {
                publishSensorData();
                mqttBatchTimer = currentTime;
                DEBUG_PRINTLN("[INVALID] MQTT данные отправлены (последние валидные)");
            }
            
            // Принудительная публикация ThingSpeak
            if (config.flags.thingSpeakEnabled)
            {
                ...
                if (tsOk)
                {
                    ...
                    DEBUG_PRINTLN("[INVALID] ThingSpeak данные отправлены (последние валидные)");
                }
                else
                {
                    DEBUG_PRINTLN("[INVALID] ThingSpeak отправка не удалась");
                }
            }
            
            invalidDataPublished = true;  // Предотвращаем повторную публикацию
        }
        else if (sensorData.valid && !dataWasValid)
        {
            // Данные снова стали валидными
            DEBUG_PRINTLN("[VALID] Данные снова стали валидными");
            invalidDataPublished = false;  // Сбрасываем флаг для будущих случаев
        }
        
        dataWasValid = sensorData.valid;  // Обновляем состояние
    }
    ...
    if (sensorData.valid && (currentTime - lastDataPublish >= config.sensorReadInterval))
    {
        ...
        // Если это первые валидные данные - публикуем немедленно
        if (!firstValidDataReceived)
        {
            firstValidDataReceived = true;
            DEBUG_PRINTLN("[FIRST] Первые валидные данные получены - немедленная публикация");
            
            // Немедленная публикация MQTT
            if (config.flags.mqttEnabled)
            {
                publishSensorData();
                mqttBatchTimer = currentTime;
                DEBUG_PRINTLN("[FIRST] MQTT данные отправлены немедленно");
            }
            
            // Немедленная публикация ThingSpeak
            if (config.flags.thingSpeakEnabled)
            {
                bool tsOk = sendDataToThingSpeak();
                if (tsOk)
                {
                    thingspeakBatchTimer = currentTime;
                    DEBUG_PRINTLN("[FIRST] ThingSpeak данные отправлены немедленно");
                }
                else
                {
                    DEBUG_PRINTLN("[FIRST] ThingSpeak отправка не удалась");
                }
            }
            
            // Сбрасываем флаги ожидания, так как уже отправили
