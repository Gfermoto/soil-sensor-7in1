...
bool sendDataToThingSpeak()
{
    ...
    // Проверяем корректность данных датчика
    if (isnan(sensorData.temperature) || isnan(sensorData.humidity) || isnan(sensorData.ph) || 
        isnan(sensorData.ec) || isnan(sensorData.nitrogen) || isnan(sensorData.phosphorus) || isnan(sensorData.potassium))
    {
        logWarn("ThingSpeak: данные датчика содержат NaN, пропускаем отправку");
        return false;
    }
    ...
    if (sensorData.temperature < -50 || sensorData.temperature > 100 ||
        ...
        sensorData.ph < 0 || sensorData.ph > 14 ||
        sensorData.ec < 0 || sensorData.ec > 10000)
    {
        logWarn("ThingSpeak: данные датчика вне разумных диапазонов, пропускаем отправку");
        return false;
    }
    ...
    // Дополнительная диагностика - логируем все значения полей
    logDebug("ThingSpeak отправка полей: Field1(T)=%s, Field2(H)=%s, Field3(EC)=%s, Field4(PH)=%s, Field5(N)=%s, Field6(P)=%s, Field7(K)=%s",
             format_temperature(sensorData.temperature).c_str(),
             format_moisture(sensorData.humidity).c_str(),
             format_ec(sensorData.ec).c_str(),
             format_ph(sensorData.ph).c_str(),
             format_npk(sensorData.nitrogen).c_str(),
             format_npk(sensorData.phosphorus).c_str(),
             format_npk(sensorData.potassium).c_str());
    ...
    // Если получили HTTP 400, попробуем альтернативный способ отправки для диагностики
    if (res == 400)
    {
        logWarn("ThingSpeak: HTTP 400 получен, пробуем альтернативный способ отправки");
        
        // Создаем HTTP клиент для диагностики
        WiFiClient client;
        HTTPClient http;
        
        String url = "https://api.thingspeak.com/update";
        url += "?api_key=" + String(apiKeyBuf);
        url += "&field1=" + String(format_temperature(sensorData.temperature).c_str());
        url += "&field2=" + String(format_moisture(sensorData.humidity).c_str());
        url += "&field3=" + String(format_ec(sensorData.ec).c_str());
        url += "&field4=" + String(format_ph(sensorData.ph).c_str());
        url += "&field5=" + String(format_npk(sensorData.nitrogen).c_str());
        url += "&field6=" + String(format_npk(sensorData.phosphorus).c_str());
        url += "&field7=" + String(format_npk(sensorData.potassium).c_str());
        
        logDebug("ThingSpeak альтернативный URL: %s", url.c_str());
        
        http.begin(client, url);
        int httpCode = http.GET();
        String payload = http.getString();
        
        logDebug("ThingSpeak альтернативный ответ: HTTP %d, тело: %s", httpCode, payload.c_str());
        
        http.end();
        
        if (httpCode == 200)
        {
            logSuccess("ThingSpeak: данные отправлены через альтернативный способ");
        ...
        {
            logError("ThingSpeak альтернативный способ тоже не работает: HTTP %d", httpCode);
        }
    }
