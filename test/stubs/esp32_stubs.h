...
public:
    ...
    String get(const char* key) const { return String(""); }
...
public:
    ...
    size_t freeBytes() { return 512 * 1024; }
    File open(const char* path, const char* mode = "r") { return File(path); }
    File open(const String& path, const char* mode = "r") { return File(path.c_str()); }
    ...
    bool rename(const String& pathFrom, const String& pathTo) { return true; }
...
public:
    ...
    bool beginTransmission(uint8_t slave) { return true; }
    bool endTransmission() { return true; }
    uint8_t requestFrom(uint8_t slave, uint8_t count) { return count; }
    uint8_t available() { return 0; }
    uint8_t read() { return 0; }
    uint8_t write(uint8_t data) { return 1; }
    uint8_t write(uint8_t* data, uint8_t length) { return length; }
    uint8_t readInputRegisters(uint8_t slave, uint16_t address, uint8_t count) { return count; }
    uint8_t readHoldingRegisters(uint8_t slave, uint16_t address, uint8_t count) { return count; }
    uint8_t writeSingleRegister(uint8_t slave, uint16_t address, uint16_t value) { return 1; }
    uint8_t writeMultipleRegisters(uint8_t slave, uint16_t address, uint8_t count) { return count; }
    uint16_t getResponseBuffer(uint8_t index) { return 0; }
...
public:
    static String generateToken() { return String("test_token"); }
    static bool validateToken(const String& token) { return true; }
    static String getTokenFromRequest(const WebServer&) { return "test_token"; }
};
...
public:
    ...
    int GET() { return 200; }
    int POST(const char* payload) { return 200; }
...
public:
    ...
    String get(int index) const { return String(""); }
