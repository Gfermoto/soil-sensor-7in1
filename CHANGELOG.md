...
- **Фильтр выбросов >2σ:** Автоматическое отбрасывание аномальных измерений в аналитике
...
- **Статистический анализ:** Расчет стандартного отклонения и Z-score для определения выбросов
...
- **Веб-интерфейс:** Новые элементы управления на странице `/intervals`
- **Производительность:** Оптимизированная сортировка для медианы (массивы до 15 элементов)

**📈 ПРОИЗВОДИТЕЛЬНОСТЬ v2.4.1:**
- **Flash:** 935KB (47.6%) - +10KB за алгоритмы фильтрации
- **RAM:** 57KB (17.4%) - стабильное потребление без дополнительных буферов
- **Фильтрация:** Минимум 3 точки для статистики, проверка каждые 30 сек
...
- Полная интеграция в веб-интерфейс и конфигурацию

---

## v2.4.0 - Базовая аналитика (11 ИЮНЯ 2025) ✅ ЗАВЕРШЕНО
...
- **Веб-интерфейс аналитики:** Новая вкладка `/analytics` с Chart.js графиками  
- **Экспорт данных:** CSV и JSON экспорт за выбранный период
- **API аналитики:** `/api/analytics` для интеграции с внешними системами

**📊 ВЕБ-ИНТЕРФЕЙС:**
- **Страница /analytics:** Профессиональная панель аналитики с графиками температуры
- **Адаптивный дизайн:** Полная поддержка мобильных устройств
- **Chart.js интеграция:** CDN загрузка для интерактивных графиков
- **Умная статистика:** Автоматический расчет каждые 30 секунд

**⚙️ ТЕХНИЧЕСКИЕ УЛУЧШЕНИЯ:**
- **Структура AnalyticsSystem:** Оптимальное управление circular buffer
- **Временные окна:** Настраиваемые периоды анализа (час/день/неделя)
- **Валидация данных:** Обработка только корректных измерений
- **Интеграция в main.cpp:** Автоматическое добавление данных при каждом измерении
...
- **Circular Buffer:** 100 точек × 33 байта = 3.3KB оптимального использования RAM
- **Скорость:** Обновление статистики каждые 30 сек без влияния на основной цикл

**📁 НОВЫЕ ФАЙЛЫ:**
- `include/analytics_system.h` - API системы аналитики
- `src/analytics_system.cpp` - Реализация аналитики и веб-интерфейса

**🔗 API ENDPOINTS:**
- `GET /analytics` - Веб-интерфейс аналитики
- `GET /api/analytics?period=hour/day/week` - JSON статистика
- `GET /api/analytics/export?format=csv/json&period=hour/day/week` - Экспорт данных
